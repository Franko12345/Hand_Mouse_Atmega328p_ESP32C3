...
DateTime::DateTime(void)
{
    ...
    debugMark(PSTR("DateTime::DateTime(void)"), Debug::CodeIndex::DATE_TIME_MODULE);
    ...
    debugMessage(Error::NONE, Debug::CodeIndex::DATE_TIME_MODULE);
    return;
}
...
{
    // Mark passage for debugging purpose
    debugMark(PSTR("DateTime::isDateSet(void)"), Debug::CodeIndex::DATE_TIME_MODULE);
...
{
    // Mark passage for debugging purpose
    debugMark(PSTR("DateTime::isTimeSet(void)"), Debug::CodeIndex::DATE_TIME_MODULE);
...
{
    // Mark passage for debugging purpose
    debugMark(PSTR("DateTime::getDate(uint16_t *, Month *, uint8_t *, WeekDay *)"), Debug::CodeIndex::DATE_TIME_MODULE);

    // Checks initialization
    if(!this->_dateSet) {
        // Returns error
        this->_lastError = Error::DATE_NOT_INITIALIZED;
        debugMessage(Error::DATE_NOT_INITIALIZED, Debug::CodeIndex::DATE_TIME_MODULE);
        return false;
    }
    ...
    if((!isPointerValid(year_p)) || (!isPointerValid(month_p)) || (!isPointerValid(day_p))) {
        ...
        debugMessage(Error::ARGUMENT_POINTER_NULL, Debug::CodeIndex::DATE_TIME_MODULE);
        return false;
    }
...
{
    // Mark passage for debugging purpose
    debugMark(PSTR("DateTime::getWeekDay(WeekDay *)"), Debug::CodeIndex::DATE_TIME_MODULE);
...
{
    // Mark passage for debugging purpose
    debugMark(PSTR("DateTime::isLeapYear(void)"), Debug::CodeIndex::DATE_TIME_MODULE);
...
{
    // Mark passage for debugging purpose
    debugMark(PSTR("DateTime::setDate(cuint16_t, const Month, cuint8_t)"), Debug::CodeIndex::DATE_TIME_MODULE);
    ...
    if(!this->_isValidDate(year_p, month_p, day_p)) {
        // Returns error
        debugMessage(this->_lastError, Debug::CodeIndex::DATE_TIME_MODULE);
        return false;
    }
...
{
    // Mark passage for debugging purpose
    debugMark(PSTR("DateTime::getTime(uint8_t *, uint8_t *, uint8_t *, const TimeFormat, AmPmFlag *)"),
    ...
    if(!this->_timeSet) {
        ...
        debugMessage(Error::TIME_NOT_INITIALIZED, Debug::CodeIndex::DATE_TIME_MODULE);
        return false;
    }
    // CHECK FOR ERROR - pointer null
...
{
    // Mark passage for debugging purpose
    debugMark(PSTR("DateTime::getTime(uint8_t *, uint8_t *, uint8_t *, uint16_t *, const TimeFormat, AmPmFlag *)"),
            Debug::CodeIndex::DATE_TIME_MODULE);
...
{
    // Mark passage for debugging purpose
    debugMark(PSTR("DateTime::getTimeFormat(TimeFormat *)"), Debug::CodeIndex::DATE_TIME_MODULE);
...
{
    // Mark passage for debugging purpose
    debugMark(PSTR("DateTime::getTimeZone(TimeZone *)"), Debug::CodeIndex::DATE_TIME_MODULE);
...
{
    // Mark passage for debugging purpose
    debugMark(PSTR("DateTime::setTime(cuint8_t, cuint8_t, cuint8_t, const TimeFormat, const AmPmFlag)"),
            Debug::CodeIndex::DATE_TIME_MODULE);
...
{
    // Mark passage for debugging purpose
    debugMark(PSTR("DateTime::setTime(cuint8_t, cuint8_t, cuint8_t, cuint16_t, const TimeFormat, const AmPmFlag)"),
            Debug::CodeIndex::DATE_TIME_MODULE);
...
bool_t DateTime::setTimeFormat(const TimeFormat timeFormat_p)
{
    ...
    // Mark passage for debugging purpose
    debugMark(PSTR("DateTime::setTimeFormat(const TimeFormat)"), Debug::CodeIndex::DATE_TIME_MODULE);
...
{
    // Mark passage for debugging purpose
    debugMark(PSTR("DateTime::setTimeZone(const TimeZone)"), Debug::CodeIndex::DATE_TIME_MODULE);
...
{
    // Mark passage for debugging purpose
    debugMark(PSTR("DateTime::_clearData(void)"), Debug::CodeIndex::DATE_TIME_MODULE);
...
{
    // Mark passage for debugging purpose
    debugMark(PSTR("DateTime::_evaluateDayOfWeek(void)"), Debug::CodeIndex::DATE_TIME_MODULE);
...
{
    // Mark passage for debugging purpose
    debugMark(PSTR("DateTime::_isLeapYear(cuint16_t)"), Debug::CodeIndex::DATE_TIME_MODULE);
...
{
    // Mark passage for debugging purpose
    debugMark(PSTR("DateTime::_isValidDate(cuint16_t, const Month, cuint8_t)"), Debug::CodeIndex::DATE_TIME_MODULE);
    ...
    if((day_p == 0) || (month_p == Month::UNDEFINED) || (year_p == 0)) {
        ...
        debugMessage(Error::DATE_INVALID, Debug::CodeIndex::DATE_TIME_MODULE);
        return false;
    }
...
{
    // Mark passage for debugging purpose
    debugMark(PSTR("DateTime::_convertTimeFormat(uint8_t *, AmPmFlag *, const TimeFormat, const TimeFormat)"),
            Debug::CodeIndex::DATE_TIME_MODULE);
    ...
    if((fromFormat_p == TimeFormat::FORMAT_12_HOURS) && (toFormat_p == TimeFormat::FORMAT_24_HOURS)) {
        ...
        default:
            ...
            debugMessage(Error::UNKNOWN, Debug::CodeIndex::DATE_TIME_MODULE);
            return false;
        }
    } else {
...
{
    // Mark passage for debugging purpose
    debugMark(PSTR("DateTime::_isValidTime(cuint8_t, cuint8_t, cuint8_t, const TimeFormat)"),
            Debug::CodeIndex::DATE_TIME_MODULE);
    ...
    if((seconds_p > 59) || (minutes_p > 59)) {
        ...
        debugMessage(Error::TIME_INVALID, Debug::CodeIndex::DATE_TIME_MODULE);
        return false;
    }
